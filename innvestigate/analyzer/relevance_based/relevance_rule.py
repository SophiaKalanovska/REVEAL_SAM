...
# from ...backend import backend as iK, graph as kgraph
...
class AlphaBetaRule(kgraph.ReverseMappingBase):
    ...
    def __init__(self,
        ...
        if copy_weights:
            ...
            if not bias and getattr(layer, "use_bias", False):
                weights = weights[:-1]
            positive_weights = [x * kutils.cast_to_floatx(x > 0) for x in weights]
            negative_weights = [x * kutils.cast_to_floatx(x < 0) for x in weights]
    ...
    def apply(self, Xs, Ys, Rs, reverse_state):
        ...
        def f(layer1, layer2, X1, X2):
            ...
            Rj_1 = kutils.to_list(grad(X1 + Z1 + Sk))
            Rj_2 = kutils.to_list(grad(X2 + Z2 + Sk))
...
class BoundedRule(kgraph.ReverseMappingBase):
    ...
    def apply(self, Xs, Ys, Rs, reverse_state):
        ...
        tmpA = kutils.to_list(grad(Xs + A + tmp))
        tmpB = kutils.to_list(grad(low + B + tmp))
        tmpC = kutils.to_list(grad(high + C + tmp))
...
class ZPlusFastRule(kgraph.ReverseMappingBase):
    ...
    def __init__(self, layer, state, copy_weights=False):
        ...
        if copy_weights:
            ...
            if layer.use_bias:
                weights = weights[:-1]
            weights = [x * kutils.cast_to_floatx(x > 0) for x in weights]
    ...
    def apply(self, Xs, Ys, Rs, reverse_state):
        ...
        # keep_positives = backend.layers.Lambda(lambda x: x * K.cast(K.greater(x,0), K.floatx()))
