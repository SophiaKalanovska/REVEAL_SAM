    ...
    * :func:`_head_mapping` defines how the outputs of the model
    ...
    def _gradient_reverse_mapping(
        ...
        if self.masks is not None:
            ...
            absolute_Ys = ibackend.apply(reverse_state["layer"], abs_max_activ)
            non_Ys = ibackend.apply(reverse_state["layer"], max_activ)

            zero_the_non_zeros_non_Ys = [ilayers.Equal_Zero()(non_Ys)]
            contribution = [ilayers.Where()([a, b, c]) for a, b, c in
                            zip(zero_the_non_zeros_non_Ys, absolute_Ys, non_Ys)]
