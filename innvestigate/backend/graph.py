...
def reverse_model(
    ...
    percent_tensors: dict[int, ReverseTensorDict]
    percent_tensors = {}

    bottleneck_tensor_ids: set[int] = set()

    def add_percent_tensors(nid, tensors_list, percent_tensors_list) -> None:
        def add_percent_tensor(i, X: Tensor, percent_X: Tensor) -> None:
            ...
            if (
                id(X) not in percent_tensors
            ):  # no duplicate entries for forward tensors
                percent_tensors[id(X)] = {
                    "nid": (nid, i),
                    "tensors": [percent_X],
                    "final_tensor": None,
                }
            else:  # more than one tensor propagating relevance to X
                percent_tensors[id(X)]["tensors"].append(percent_X)

        tmp = zip(tensors_list, percent_tensors_list)
        for i, (X, percent_X) in enumerate(tmp):
            add_percent_tensor(i, X, percent_X)

    def get_percent_tensor(tensor: Tensor) -> Tensor:
        tmp: ReverseTensorDict
        tmp = percent_tensors[id(tensor)]
    ...
    if forward_contibution:
        layers, reverse_execution_list, outputs, inputs = execution_trace
    ...
    if forward_contibution:
        upper_bound = None
        lower_bound = None
        p = [tf.concat(random_masks, 0)]
        masks_size = len(random_masks)

        for _nid, (layer, Xs, Ys) in enumerate(reverse_execution_list):
            if isinstance(layer, klayers.InputLayer):
                next_Xs = [tf.keras.layers.Multiply()([a, b])
                           for a, b in zip(Xs, p)]
                reversed_Xs = ibackend.to_list(next_Xs)
                add_reversed_tensors(_nid, Xs, reversed_Xs)
                add_percent_tensors(_nid, Xs, p)
                ...
                Xs, Ys = ibackend.to_list(Xs), ibackend.to_list(Ys)
                if not all(id(X) in reversed_tensors for X in Xs):
                    ...
                    continue
                reversed_Xs = [get_reversed_tensor(X) for X in Xs]
                ...
                if isinstance(reversed_Ys, tuple):
                    forward_RYs = reversed_Ys[0]
                    percent_ys = reversed_Ys[1]
                else:
                    forward_RYs = ibackend.to_list(reversed_Ys)
                    add_reversed_tensors(_nid, Ys, forward_RYs)
                    continue
                #     lower_bound = reversed_Xs[2]
                #     reversed_Ys = ibackend.to_list(reversed_Ys)
                # else:
                reversed_Ys = ibackend.to_list(forward_RYs)
                add_reversed_tensors(_nid, Ys, reversed_Ys)
                percent_ys = ibackend.to_list(percent_ys)
                add_percent_tensors(_nid, Ys, percent_ys)

        # Return requested values
        reversed_input_tensors = [
            get_reversed_tensor(tmp)
            for tmp in model.outputs
            if id(tmp) not in stop_mapping_at_ids
        ]
        if return_all_reversed_tensors is True:
            return reversed_input_tensors, percent_tensors
