...
class PerturbationAnalysis:
    ...
    def evaluate_generator(self, generator, steps=None,
        ...
        For documentation, refer to backend.engine.training.evaluate_generator (https://keras.io/models/model/)
        """
        ...
        if not is_sequence and use_multiprocessing and workers > 1:
            warnings.warn(
                UserWarning('Using a generator with `use_multiprocessing=True`'
                            ...
                            ' Please consider using the`backend.utils.Sequence'
                            ' class.'))
        if steps is None:
            if is_sequence:
                steps = len(generator)
                ...
                raise ValueError('`steps=None` is only valid for a generator'
                                 ' based on the `backend.utils.Sequence` class.'
                                 ' Please specify `steps` or use the'
                                 ' `backend.utils.Sequence` class.')
        enqueuer = None
